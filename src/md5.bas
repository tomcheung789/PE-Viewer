...
sub  md5.MD5Init(byval ctx as MD5Context ptr)
...
sub md5.MD5Update(byval ctx as MD5Context ptr, byval buf as ubyte ptr, byval llen as unsigned integer)
...
''
sub md5.MD5Final(byval digest as ubyte ptr, byval ctx as MD5Context ptr)
...
sub md5.MD5Transform (byval buf as integer ptr, byval in as integer ptr)
...
Function md5.MD5_FromByte(st() As UByte) As String
	...
	MD5Init( @ctx )
	MD5Update( @ctx, @st(0), UBound(st)+1 )
...
Function md5.MD5_FromString(st As String) As String
	Var st_len = Len(st)
...
Function md5.MD5_FromFile(filename As String) As String
	Var f = FreeFile
